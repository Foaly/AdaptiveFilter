...
Wie schon in der Untersuchung des vorhergehenden Systems ist der RLS im Vergleich zum LMS weniger empfindlich gegen hinzugefügtes Rauschen als der LMS.

Eine Auffälligkeit im Vergleich zur vorhergehenden Untersuchung des FIR Systems, ist das beide Algorithmen nicht auf die korrekten Werte des IIR Systems erreichen.
Die beiden Algorithmen konvergieren auf andere Werte und approximieren mit diesen lediglich die Filterkoeffizienten des IIR Systems.
Theoretisch ist es dem LMS und RLS Algorithmus möglich das vorher untersuchte FIR System korrekt abzubilden, in dem die exakten Filterkoeffizienten des Systems erreicht werden. 
Die ist den beiden verwendeten Algorithmen bei einem IIR System nicht möglich, da ein IIR System einen rekursiven Anteil enthält, den diese nicht abbilden können.
Deshalb ist nur die hier beobachtet Approximation des Systems möglich.
...
Betrachtet man den LMS Algorithmus im Vergleich mit dem RLS ohne eine korrekte Einstellung des Vergessensfaktor, so ist deutlich zu sehen, das er bei einem Systemwechsel eine deutliche kürze Adaptionszeit hat um sich an die neuen Filterkoeffizienten anzupassen.
