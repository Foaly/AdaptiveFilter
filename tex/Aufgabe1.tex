...
\begin{figure}[H]
 ...
 \caption{Blockschaltbild des zu untersuchenden FIR Systems ($h_0 = 0.7, h_1 = 0.1, h_2 = -0.03, h_3 = 0.18, h_4 = -0.24$).}
...
Da die Schrittweite $\mu$ und der Vergessensfaktors $\rho$ Eigenschaften des LMS respektive des KLS Algorithmus sind und nicht Eigenschaften des Signals, sind die Einflüsse dieser Parameter identisch zu deren Beschreibung in \ref{sec:FIR_mu} und \ref{sec:FIR_rho}.
...
Es ist zu beachten, dass es unter idealen Bedingungen (kein Rauschen, korrekte Parameter etc.) möglich ist ein FIR Filter mithilfe des LMS und KLS Algorithmus perfekt nachzubilden.
Dies wurde in Abschnitt \ref{sec:iir} approximiert.
Bei einem IIR System ist dies jedoch nicht exakt möglich, da es eine rekursiven Anteil besitzt, welcher in den LMS und KLS Algorithmen nicht abgebildet wird.
Auf Grundlage dessen ist mit den beiden Algorithmen nur eine Approximation des IIR Systems möglich.
Dies erklärt auch warum die exakten Filtergewichte aus \ref{figIIR_System} selbst bei einem hohen Wert für $N$ nicht erreicht wurden.
